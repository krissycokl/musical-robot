...
public class FlooringView {
    ...
    public int getOrderFromList() {
        return io.getInt("Enter # of desired order.", true);
    }
    ...
    public String getOrderMaterial(String material, List<Material> validMaterials) {
        ...
        io.print(
                ...
                + String.format("%11.11s", "Mat cost |")
                + String.format("%11.11s", " Labor cost")
        );
        IntStream.range(0, validMaterials.size()).forEach(index -> {
            Material currMat = validMaterials.get(index);
            String currLine = String.format("%-14.14s", (index + 1) + ".) " + currMat.getName());
            currLine += "|";
            currLine += String.format("%9.9s", " $" + currMat.getMaterialCost());
            currLine += " |";
            currLine += String.format("%10.10s", " $" + currMat.getLaborCost());
            io.print(currLine);
        });
        int choice = io.getInt(prompt, true, 1, validMaterials.size());
        if (choice == 0) {
            return material;
        }
    ...
    public String getOrderState(String state, List<State> validStates) {
        ...
        IntStream.range(0, validStates.size()).forEach(index -> {
            ...
            String currLine = String.format("%-9.9s", (index + 1) + ".) " + currState.getName());
            currLine += "|";
            currLine += String.format("%8.8s", currState.getTaxRate());
            io.print(currLine);
        });
        int choice = io.getInt(prompt, true, 1, validStates.size());
        if (choice == 0) {
            return state;
        }
